...
# 6. Cron Jobs (Running as www-data to match web server permissions) * Log Rotation
echo "--> Setting up Cron Polling and Log Rotation..."
...
# Run multithreaded poller wrapper every 5 minutes (Using the new 'wrapper' command)
...
cat <<EOF > /etc/logrotate.d/observium
